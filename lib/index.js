 ...
 *
 * The opencode Zen gateway (https://opencode.ai/zen/go/v1) streams chat
 * completions with a non-standard terminator: it never sends a
 * `finish_reason` and never sends the `data: [DONE]` SSE sentinel — the
 * HTTP body just closes, typically after a final chunk like
 * {"choices":[],"cost":"0"}. pi-ai's openai-completions adapter treats
 * that as a truncated stream and emits `Stream ended without
 * finish_reason`, which turns an otherwise complete response into a failed
 * turn.
 *
 * This plugin intercepts the llm/stream waterfall and rewrites that exact
 * terminal error finish into a normal stop finish — but ONLY for provider
 * routes that are opencode-based (provider id containing "opencode", or a
 * route whose llm-pi-ai settings baseURL contains "opencode.ai"). Every
 * other provider keeps the strict truncation detection.
 */
 ...
 * Matches by route id first (covers opencode / opencode-go / opencode-go-ext
